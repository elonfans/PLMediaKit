...
#import "NSString+PLAddition.h"
#import "PLPLGlobal.h"
#import <CommonCrypto/CommonDigest.h>
#import "NSData+PLBase64.h"
@implementation NSString(PLAddition)
...
//    QBLogDebug(@"striped lines %d",ary.count - rst.count);
...
- (NSString *)URLEscaped {
	...
	NSString *out = [NSString stringWithString:(NSString *)CFBridgingRelease(escaped)];
	return [out copy];
}

- (NSString *)unURLEscape {
	CFStringRef unescaped = CFURLCreateStringByReplacingPercentEscapes(NULL, (CFStringRef)self, (CFStringRef)@"");
	NSString *out = [NSString stringWithString:(__bridge NSString *)unescaped];
	return [out copy];
}
