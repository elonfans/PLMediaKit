...
@interface PLSDAVAssetExportSession ()
...
@implementation PLSDAVAssetExportSession
{
    NSError *_error;
...
{
    return [PLSDAVAssetExportSession.alloc initWithAsset:asset];
}
    ...
    while (input.isReadyForMoreMediaData)
    {
        ...
        if (sampleBuffer)
        {
            ...
            if (self.reader.status != AVAssetReaderStatusReading || self.writer.status != AVAssetWriterStatusWriting)
            {
                handled = YES;
                error = YES;
                NSLog(@"error1 %@ %@", self.writer.error, input);
            ...
            {
                error = YES;
                NSLog(@"error2 %@ %@", self.writer.error, input);
