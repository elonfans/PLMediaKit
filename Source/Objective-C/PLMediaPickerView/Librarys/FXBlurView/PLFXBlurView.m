...
//
//  https://github.com/nicklockwood/PLFXBlurView
//
...
@implementation UIImage (PLFXBlurView)
...
@interface PLFXBlurScheduler : NSObject
...
@interface PLFXBlurLayer: CALayer
...
@implementation PLFXBlurScheduler

+ (instancetype)sharedInstance
{
    static PLFXBlurScheduler *sharedInstance = nil;
    if (!sharedInstance)
    {
        sharedInstance = [[PLFXBlurScheduler alloc] init];
    }
...
- (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    if (blurEnabled)
    {
        for (PLFXBlurView *view in self.views)
        {
...
- (void)addView:(PLFXBlurView *)view
{
...
- (void)removeView:(PLFXBlurView *)view
{
...
- (void)updateAsynchronously
{
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        {
            PLFXBlurView *view = self.views[i];
...
@implementation PLFXBlurView

+ (void)setBlurEnabled:(BOOL)blurEnabled
{
    [PLFXBlurScheduler sharedInstance].blurEnabled = blurEnabled;
}

+ (void)setUpdatesEnabled
{
    [[PLFXBlurScheduler sharedInstance] setUpdatesEnabled];
}

+ (void)setUpdatesDisabled
{
    [[PLFXBlurScheduler sharedInstance] setUpdatesDisabled];
}

+ (Class)layerClass
{
    return [PLFXBlurLayer class];
}
...
- (PLFXBlurLayer *)blurLayer
{
    return (PLFXBlurLayer *)self.layer;
}

- (PLFXBlurLayer *)blurPresentationLayer
{
    PLFXBlurLayer *blurLayer = [self blurLayer];
    return (PLFXBlurLayer *)blurLayer.presentationLayer ?: blurLayer;
}
...
- (void)schedule
{
    ...
    {
        [[PLFXBlurScheduler sharedInstance] addView:self];
    }
    else
    {
        [[PLFXBlurScheduler sharedInstance] removeView:self];
    }
}
...
- (BOOL)shouldUpdate
{
    ...
    self.blurEnabled && [PLFXBlurScheduler sharedInstance].blurEnabled &&
...
- (UIImage *)snapshotOfUnderlyingView
{
    __strong PLFXBlurLayer *blurLayer = [self blurPresentationLayer];
    __strong CALayer *underlyingLayer = [self underlyingLayer];
